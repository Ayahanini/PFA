...
class CardiacTest {
    ...
    renderResults() {
        ...
        if (riskLevelElement) {
            ...
            riskLevelElement.textContent = riskTexts[this.testResults.riskLevel];
            riskLevelElement.className = `risk-level ${this.testResults.riskLevel}`;
        }

        if (riskPercentageElement) {
            riskPercentageElement.textContent = `${this.testResults.percentage}%`;
        }

        if (riskFactorsElement) {
            riskFactorsElement.innerHTML = this.testResults.factors
                .map(factor => `<li>${factor}</li>`)
                .join('');
        }

        if (recommendationsElement) {
            recommendationsElement.innerHTML = this.testResults.recommendations
                .map(rec => `<li>${rec}</li>`)
                .join('');
        }

        // Mettre à jour la barre de progression des résultats
        const progressBar = document.querySelector('#results .progress-bar');
        if (progressBar) {
            progressBar.style.width = `${this.testResults.percentage}%`;
            progressBar.className = `progress-bar ${this.testResults.riskLevel}`;
        }
    }
